...
public class MapsActivity_tests extends ActivityInstrumentationTestCase2<MapsActivity>  implements Constants {
    ...
    public void test_firstLocationSave() {
        ...
        mapsActivity.addLocation(Integer.toString(R.string.loc0), testPoints[0]);

        assertEquals(mapsActivity.locList.get(0).getCoords(), testPoints[0]);
        assertEquals(mapsActivity.locList.size(), 1);
    }
    ...
    public void test_tooCloseNoAdd() {
        ...
        mapsActivity.addLocation(Integer.toString(R.string.loc0), testPoints[0]);
        mapsActivity.addLocation(Integer.toString(R.string.loc1), testPoints[1]);

        assertEquals(mapsActivity.locList.get(0).getCoords(), testPoints[0]);
        assertEquals(mapsActivity.locList.get(1).getCoords(), testPoints[1]);
        assertNotNull(mapsActivity.locList.get(1));
        assertEquals(mapsActivity.locList.size(), 2);
    }
    ...
    public void test_commonSaveLoc() {
        ...
        mapsActivity.addLocation(Integer.toString(R.string.loc0), testPoints[0]);
        mapsActivity.addLocation(Integer.toString(R.string.loc1), testPoints[2]);

        assertEquals(mapsActivity.locList.get(0).getCoords(), testPoints[0]);
        assertEquals(mapsActivity.locList.get(1).getCoords(), testPoints[2]);
        assertNotNull(mapsActivity.locList.get(1));
        assertEquals(mapsActivity.locList.size(), 2);
    }
    ...
    public void test_reAddDeleted() {
        ...
        mapsActivity.addLocation(Integer.toString(R.string.loc0), testPoints[0]);
        mapsActivity.addLocation(Integer.toString(R.string.loc3), testPoints[3]);

        assertEquals(mapsActivity.locList.get(0).getCoords(), testPoints[0]);
        assertEquals(mapsActivity.locList.get(1).getCoords(), testPoints[3]);
        assertEquals(mapsActivity.locList.size(), 2);

        mapsActivity.removeLocation(Integer.toString(R.string.loc0));

        assertEquals(mapsActivity.locList.size(), 1);

        mapsActivity.removeLocation(Integer.toString(R.string.loc3));

        assertEquals(mapsActivity.locList.size(), 0);
    }
}
